...
system_create_user() {
  ...
  sudo su - root <<EOF
  useradd -m -p $(openssl passwd -crypt ${mysql_root_password}) -s /bin/bash -G sudo espochat
  usermod -aG sudo espochat
EOF
...
system_git_clone() {
  ...
  git clone ${link_git} /home/espochat/${instancia_add}/
EOF
...
sudo su - espochat <<EOF
 rm -rf /home/espochat/${empresa_delete}
...
sleep 2
  ...
  sudo su - espochat <<EOF
  cd && cd /home/espochat/${empresa_dominio}/frontend
  sed -i "1c\REACT_APP_BACKEND_URL=https://${alter_backend_url}" .env
  cd && cd /home/espochat/${empresa_dominio}/backend
...
cat > /etc/nginx/conf.d/espochat.conf << 'END'
